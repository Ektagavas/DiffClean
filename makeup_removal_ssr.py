...
class DiffClean_SSR(object):
    ...
    def clip_finetune(self):
        ...
        age_loss_func = AgeLoss(self.device)
        ...
        for mode in ['train', 'test']:
            ...
            #                           f'{self.config.data.category}_{mode}_t{self.args.t_0}_nim{self.args.n_precomp_img}_ninv{self.args.n_inv_step}_sch{self.args.schedule}_pairs.pth')
            if mode == 'train':
            ...
            if self.args.do_train:
                if os.path.exists(save_name):
                    ...
                    for step, (x0, x_id, x_lat, x_nm, gtage, nomkage) in enumerate(img_lat_pairs_dic['train']):
                        ...
                        loss_age = loss_age/len(seq_train)
