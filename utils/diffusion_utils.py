...
def get_beta_schedule(*, beta_start, beta_end, num_diffusion_timesteps, name="linear"):
    if name == "linear":
        print('Using Linear Beta Noise schedule')
        betas = np.linspace(beta_start, beta_end,
                            num_diffusion_timesteps, dtype=np.float64)
        assert betas.shape == (num_diffusion_timesteps,)
        return betas
    elif name == "cosine":
        print('Using Cosine Beta Noise schedule')
