...
class CLIPLoss(torch.nn.Module):
    def __init__(self, device, lambda_makeup_direction , lambda_direction, lambda_age, direction_loss_type='cosine', clip_model='ViT-B/32'):
        ...
        self.model, clip_preprocess = clip.load(clip_model, download_root='./clip_models', device=self.device)
    ...
    def age_clip_loss(self, image: torch.Tensor, text: str):
        image_features = self.get_image_features(image)
        image_features /= (image_features.clone().norm(dim=-1, keepdim=True) + 1e-7)
        text_features = self.get_text_features(text).mean(axis=0, keepdim=True)
        text_features /= text_features.norm(dim=-1, keepdim=True)
        return self.direction_loss(image_features, text_features).mean()
    ...
    def forward(self, src_img: torch.Tensor, non_makeup_img: torch.Tensor, output_img: torch.Tensor, makeup_img: torch.Tensor, src_txt = None, trg_txt = None):
        ...
        elif self.lambda_age == 1:
            clip_loss += self.age_clip_loss(src_img, src_txt)
        return clip_loss
